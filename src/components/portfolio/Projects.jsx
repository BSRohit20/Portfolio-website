...
const Projects = ({ data }) => {
  return (
    <div className="max-w-106 rounded-lg outline-[#FFFFFF] hover:shadow-2xl duration-300 transition-all shadow-gray-300 border border-gray-200">
      <img src={data?.image} alt={`${data?.title} image`} className="w-full h-64 object-cover rounded-t-lg" />
      <div className="p-4 xs:p-8">
        <p className="text-picto-primary text-xs font-semibold uppercase tracking-wide">{data?.category}</p>
